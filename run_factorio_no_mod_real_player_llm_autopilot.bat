...
set PYTHONPATH=src
set FACTORIO_AI_AGENT_PLAYER=auto
set FACTORIO_AI_REQUIRE_REAL_PLAYER=1
set FACTORIO_AI_USE_GUI_INPUT_FOR_MOVEMENT=1
...
echo [factorio-ai] Opening GUI client. This real-player autopilot will use the first connected player.
python -m factorio_ai.cli launch-no-mod-gui --window-size 1600x900 || exit /b 1
echo [factorio-ai] Waiting for the GUI player character to join...
timeout /t 10 /nobreak >nul

echo [factorio-ai] Starting no-custom-mod autopilot in strict real-player mode.
echo [factorio-ai] If no connected character is available, the loop will fail instead of using the virtual server agent.
